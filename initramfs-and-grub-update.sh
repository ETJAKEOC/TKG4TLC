...
echo "#####################################################"
echo "ETJAKEOC TKG kernel initramfs and GRUB2 update script"
echo "#####################################################"
echo "#####################################################"
...
# Find the TKG kernel package
tkg_k=$(find /lib/modules -name 'kernel*tgk*')

if [ -z "$tkg_k" ]; then
    echo "Error: TKG kernel package not found."
    exit 1
fi
...
if [ -z "$tkg_version" ]; then
    echo "Error: Unable to extract TKG kernel version."
    exit 1
fi
